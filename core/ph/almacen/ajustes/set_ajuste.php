<?php
	...
	$db->query('SELECT  MVTT_CNSCTV+1 AS INCREMENTO,MVTT_CLV FROM MOVIMIENTOS_TIPOS WHERE MVTT_IDINTRN = 3');
	$row = $db->single(); 
...
:MAL_FL
	...
	$db->bind(':MOVIMIENTOS_TIPOS_MVTT_IDINTRN',3);
	...
	if($db->lastId()){
			...
			if ($tipo > 1) { 
				$db->bind(':MVTD_MTV', "Ajuste Negativo");  
			}else{ 
				$db->bind(':MVTD_MTV', "Ajuste Positivo");  
			}
