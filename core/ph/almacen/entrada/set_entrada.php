<?php
	...
	$db->query('SELECT  MVTT_CNSCTV+1 AS INCREMENTO,MVTT_CLV FROM MOVIMIENTOS_TIPOS WHERE MVTT_IDINTRN = 1');
	$row = $db->single(); 
	...
	$db->bind(':MOVIMIENTOS_TIPOS_MVTT_IDINTRN',1);
	...
	$db->bind(':MAL_MTV', "Entrada por compra");
	$db->bind(':MOVIMIENTOS_TIPOS_MVTT_IDINTRN',1);
	...
	if($db->lastId()){
		...
		for ($i=0; $i < $num; $i++) {  
			...
			$db->bind(':MVTD_MTV', "Compra de mercancia");  
