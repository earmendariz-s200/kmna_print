<?php
	...
	$db->query('SELECT 
				...
				WHERE MOVIMIENTOS_ALMACEN.MOVIMIENTOS_TIPOS_MVTT_IDINTRN = 4 '.$where);
					$row = $db->fetch();
	if(count($row) > 0){
		...
		foreach ($row as $key => $value) {
			...
			if ($value["MAL_FL1"] != "" && $value["MAL_FL1"] !== null) { 
				...
				if (file_exists($url_nota)) {
					$file_nota = "<a href='".$url_nota."' ><i class='fa fa-file-text'></i> <span>". $value["MAL_NMBR_FL1"]. "</span></a>"; 
				}else{
			...
			if ($value["MAL_FL2"] != "" && $value["MAL_FL2"] !== null) { 
				...
				if (file_exists($url_factura)) {
					$file_factura = "<a href='".$url_factura."' ><i class='fa fa-file-text'></i> <span>". $value["MAL_NMBR_FL2"]. "</span> </a>"; 
				}else{
